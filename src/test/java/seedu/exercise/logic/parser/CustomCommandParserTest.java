...
import static seedu.exercise.logic.commands.CommandTestUtil.INVALID_SHORT_NAME_DESC;
...
import static seedu.exercise.logic.commands.CommandTestUtil.SHORT_NAME_DESC_ENDDATE;
import static seedu.exercise.logic.commands.CommandTestUtil.SHORT_NAME_DESC_RATING;
import static seedu.exercise.logic.commands.CommandTestUtil.SHORT_NAME_DESC_REMARK;
...
import static seedu.exercise.model.exercise.CustomProperty.FULL_NAME_CONSTRAINTS;
import static seedu.exercise.model.exercise.CustomProperty.SHORT_NAME_CONSTRAINTS;
import static seedu.exercise.model.exercise.ParameterType.PARAMETER_CONSTRAINTS;
...
class CustomCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + SHORT_NAME_DESC_RATING
        ...
        assertParseSuccess(parser, SHORT_NAME_DESC_ENDDATE + SHORT_NAME_DESC_RATING
                        + FULL_NAME_DESC_RATING + PARAMETER_TYPE_DESC_RATING,
                new CustomCommand(expectedCustomProperty));

        // multiple full names - last full name accepted
        assertParseSuccess(parser, SHORT_NAME_DESC_RATING + FULL_NAME_DESC_ENDDATE
                        + FULL_NAME_DESC_RATING + PARAMETER_TYPE_DESC_RATING,
                new CustomCommand(expectedCustomProperty));

        // multiple parameter types - last parameter type accepted
        assertParseSuccess(parser, SHORT_NAME_DESC_RATING + FULL_NAME_DESC_RATING
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        assertParseFailure(parser, VALID_SHORT_NAME_RATING + FULL_NAME_DESC_RATING
                + PARAMETER_TYPE_DESC_RATING, expectedMessage);

        // missing full name prefix
        assertParseFailure(parser, SHORT_NAME_DESC_RATING + VALID_FULL_NAME_RATING
                + PARAMETER_TYPE_DESC_RATING, expectedMessage);

        // missing parameter type prefix
        assertParseFailure(parser, SHORT_NAME_DESC_RATING + FULL_NAME_DESC_RATING
                + VALID_PARAMETER_TYPE_RATING, expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_SHORT_NAME_RATING + VALID_FULL_NAME_RATING
                + VALID_PARAMETER_TYPE_RATING, expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid short name
        assertParseFailure(parser, INVALID_SHORT_NAME_DESC + FULL_NAME_DESC_RATING
                + PARAMETER_TYPE_DESC_RATING, SHORT_NAME_CONSTRAINTS);

        // invalid full name
        assertParseFailure(parser, SHORT_NAME_DESC_REMARK + INVALID_FULL_NAME_DESC
                + PARAMETER_TYPE_DESC_REMARK, FULL_NAME_CONSTRAINTS);

        // invalid parameter type
        assertParseFailure(parser, SHORT_NAME_DESC_REMARK + FULL_NAME_DESC_REMARK
                + INVALID_PARAMETER_TYPE_DESC, PARAMETER_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_SHORT_NAME_DESC + INVALID_FULL_NAME_DESC
                + PARAMETER_TYPE_DESC_RATING, SHORT_NAME_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + SHORT_NAME_DESC_RATING
                        + FULL_NAME_DESC_RATING + PARAMETER_TYPE_DESC_RATING,
